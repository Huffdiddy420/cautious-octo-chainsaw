...
class USBankRoutingNumberConfig() : TextFieldConfig {
    override val capitalization: KeyboardCapitalization = KeyboardCapitalization.None
    override val debugLabel = "us_bank_routing_number"

    @StringRes
    override val label = R.string.us_bank_account_widget_routing_number
    ...
    override fun determineState(input: String): TextFieldState {
        ...
        if (input.length < LENGTH) {
            return TextFieldStateConstants.Error.Incomplete(
                R.string.us_bank_account_widget_routing_number_incomplete
            )
        }
        ...
        if (bank == null || input.length > LENGTH) {
            return TextFieldStateConstants.Error.Invalid(
                R.string.us_bank_account_widget_routing_number_invalid
            )
        }
